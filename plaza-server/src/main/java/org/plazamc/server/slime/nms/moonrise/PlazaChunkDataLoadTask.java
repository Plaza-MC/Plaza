...
import org.plazamc.server.slime.nms.PlazaCommonLoadTask;
import org.slf4j.Logger;

import java.util.function.Consumer;

public final class PlazaChunkDataLoadTask implements PlazaCommonLoadTask {
    ...
    public PlazaChunkDataLoadTask(ChunkLoadTask chunkLoadTask, final ChunkTaskScheduler scheduler, final ServerLevel world,
                                  final int chunkX, final int chunkZ, final Priority priority,
                                  final Consumer<GenericDataLoadTask.TaskResult<ChunkAccess, Throwable>> onRun) {
        ...
        this.task = this.scheduler.createChunkTask(this.chunkX, this.chunkZ, () -> {
            try {
                PlazaSlimeChunk chunk = this.world.slimeInstance.getChunk(this.chunkX, this.chunkZ);
    ...
    private ChunkAccess getEmptyChunk() {
        ...
        return new ImposterProtoChunk(new LevelChunk(this.world, new ChunkPos(this.chunkX, this.chunkZ), UpgradeData.EMPTY,
                blockLevelChunkTicks, fluidLevelChunkTicks, 0L, null, chunk -> {}, null), true);
    }

    private ChunkAccess runOnMain(final PlazaSlimeChunk data) {
        try {
